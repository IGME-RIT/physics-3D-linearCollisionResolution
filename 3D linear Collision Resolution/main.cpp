...
void setup()
{
	...
	float radius = 0.1f;
	sphere1.radius = size1 * 0.1f;
	sphere2.radius = size2 * 0.1f;
	sphere1.mass = size1;
	sphere2.mass = size2;

	sphere1.velocity = glm::vec3(0.0f, 0.0f, 0.0f);
	sphere2.velocity = glm::vec3(-0.04f, 0.03f, 0.0f);

	sphere1.RotationMat = glm::mat4(1.0f);
	sphere2.RotationMat = glm::mat4(1.0f);
	sphere1.scaleMat = glm::scale(glm::vec3(size1, size1, size1));
	sphere2.scaleMat = glm::scale(glm::vec3(size2, size2, size2));

	sphere1.MVP = PV * (glm::translate(sphere1.origin) * (sphere1.RotationMat * sphere1.scaleMat));
	sphere2.MVP = PV * (glm::translate(sphere2.origin) * (sphere2.RotationMat * sphere2.scaleMat));
...
{
	// Before proceeding to solving the linear collision in 3D,
	// lets solve the elastic collision in 1 Dimension first
	/* for two objects of mass m1 and m2 moving at velocity u1 and u2 
	to collide and move with velocities v1 and v2. they need to follow two simple rules:

	Conservation of Momentum: m1u1 + m2u2 = m1v1 + m2v2
	and 
	Conservation of Energy	: m1u1^2 + m2u2^2 = m1v1^2 + m2v2^2

	Solving these two linear equations will give us the the velocities V1 nad V2

	v1 = (m1-m2)u1 + 2m2u2 / (m1 + m2)
	v2 = 2m1u1 + (m2-m1)u2 / (m1 + m2)
	*/
	glm::vec3 An, Bn, Apn, Bpn;
	
	// To solve the collisions in 3D, we have to convert it to 1D
	// So, we take the component of velocities in that 1 direction: we store it in An and Bn.
	glm::vec3 n = glm::normalize(B.origin - A.origin);

	An = glm::dot(A.velocity, n) * n;
	Bn = glm::dot(B.velocity, n) * n;
...
void bound(Sphere &S)
{
	if (abs(S.origin.x) > 1.0f)
		S.velocity.x *= (-1.0f);
	if (abs(S.origin.y) > 1.0f)
		S.velocity.y *= (-1.0f);
	if (abs(S.origin.z) > 1.0f)
		S.velocity.z *= (-1.0f);
}
...
void update()
{
	...
	if (returnMTV(*boxInfocus, *boxOutfocus, n))		// Check if the collision occours
	{
		...
		{
			num = glm::length(boxInfocus->velocity) / (glm::length(boxInfocus->velocity) + glm::length(boxOutfocus->velocity));
			denom = glm::length(boxInfocus->velocity) / (glm::length(boxInfocus->velocity) + glm::length(boxOutfocus->velocity));
			
			boxInfocus->origin -= n*num;
			boxOutfocus->origin += n*denom;						// if collision occours then move the other obejct by MTV

			resolveCollision(*boxInfocus, *boxOutfocus);
		}
	}
	
	sphere1.origin = EulerIntegrator(sphere1.origin, timeStep, sphere1.velocity);
	sphere2.origin = EulerIntegrator(sphere2.origin, timeStep, sphere2.velocity);
	bound(sphere1);
	bound(sphere2);
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	{
		system("cls");
		std::cout << "enter the velocity of the selected object :";
		std::cout << "velocity in X axis (0-10):";
		std::cin >> boxInfocus->velocity.x;
		std::cout << "velocity in Y axis (0-10):";
		std::cin >> boxInfocus->velocity.y;
		std::cout << "velocity in Z axis (0-10):";
		std::cin >> boxInfocus->velocity.z;

		boxInfocus->velocity *= 0.01f;

		system("cls");
		std::cout << "\n This program demonstrates the linear collision resolution between two spheres\n\n\n\n\n\n\n\n\n\n";
		...
		std::cout << "\n Use \"R\" to reset the objects to their default position.";
		std::cout << "\n use \"ENTER\" to input the velocity of the selected obejct \n in the comand window.";
		std::cout << "\n You can use the first block of code in the main.cpp file to change the sizes of the spheres";
	}

	//Reset the positions of the obejcts and set their velocities to 0
	if (key == GLFW_KEY_R && (action == GLFW_PRESS || action == GLFW_REPEAT))
	{
		...
		sphere1.velocity = glm::vec3(0.0f, 0.0f, 0.0f);
		sphere2.velocity = glm::vec3(0.0f, 0.0f, 0.0f);
	}
...
void main()
{
	...
	window = glfwCreateWindow(800, 800, "3D Linear collision resolution", nullptr, nullptr);
